...
function gainEnergy(stateObj, energyToGain, energyCost=false) {
  stateObj = immer.produce(stateObj, (newState) => {
    newState.playerMonster.encounterEnergy += energyToGain;
    if (energyCost) {
      newState.playerMonster.encounterEnergy -= energyCost
    }
  })
...
async function drawAHand(stateObj) {
  ...
  let cardsLeft = 7 - stateObj.encounterHand.length
  let cardsToDraw =  (cardsLeft > (stateObj.playerMonster.turnCards-1)) ? stateObj.playerMonster.turnCards-1 : cardsLeft
