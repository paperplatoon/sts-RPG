...
let easySoloEncounters = {
          ...
          },
          {
            ...
            action: async (stateObj, index, array) => {
              ...
              stateObj = immer.produce(stateObj, (newState) => {
                newState.opponentMonster[index].encounterBlock += ((array[index].baseBlock)  + array[index].dex);
                newState.opponentMonster[index].strength += Math.floor(array[index].baseScale);
                newState.opponentMonster[index].dex += Math.floor(array[index].baseScale);
      ...
      e3: {
        ...
        moves: [
          ...
          },
          {
            ...
            energyChange: "-2",
            action: async (stateObj, index, array) => {
              stateObj = await dealPlayerDamage(stateObj, (array[index].baseDamage*2) + 1, index, -2);
              return stateObj;
            }
          },
    ]
  ...
  e4: {
    ...
    opponentMoveIndex: false,
    currentHP: opponentMaxHP*8,
    inflame: Math.floor(opponentBaseDamage/2),
    strength: 0,
    dex: 0,
    drown: 0,
    hunted: 0,
    ...
    moves: [
      {
        ...
        cost: "0",
        text: (state, index, array) => {
            return `Gain ${Math.floor(array[index].baseScale)} strength.`
        },
        minReq: 0,
        energyChange: "+3",
        action: async (stateObj, index, array) => {
    ...
    moves: [
      {
        ...
        cost: "0",
        text: (state, index, array) => {
          return `Deal ${Math.floor(array[index].baseDamage) + array[index].strength}  damage. Restore ${Math.floor(array[index].baseHeal)} health`
        },
        minReq: 0,
        energyChange: "+1",
        action: async (stateObj, index, array) => {
